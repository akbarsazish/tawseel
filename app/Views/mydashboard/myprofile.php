<div class="card mb-4">
    <div class="card-header">
        <h2 class="h4 mb-0"><?= lang('app.myprofile') ?></h2>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= !empty($record['province']) ? lang('app.provinces')[$record['province']] ?? '' : '' ?></td>
                    </tr>
                    <tr>
                        <th><?= lang('app.city') ?></th>
                        <td>
            ...
            <?php if (!empty($record['latitude']) && !empty($record['longitude'])): ?>
                <div class="map-container" style="border: 1px solid #dee2e6; border-radius: 0.25rem; overflow: hidden;">
                    <img 
    src="https://maps.googleapis.com/maps/api/staticmap?center=<?= $record['latitude'] ?>,<?= $record['longitude'] ?>&zoom=14&scale=2&size=1600x200&maptype=roadmap&markers=color:red%7C<?= $record['latitude'] ?>,<?= $record['longitude'] ?>&key=<?= env('GOOGLE_MAPS_API_KEY') ?>" 
    alt="Location Map" 
...
>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6 d-flex justify-content-end">
                    <button type="button" class="btn btn-primary px-4" onclick="loadMe('<?= base_url('mydashboard/myprofile/edit') ?>');">
