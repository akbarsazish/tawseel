...
<html lang="<?=sess()->get('lang')?>">
  ...
  <title><?= $this->renderSection('title') ?></title>
    ...
    <link rel="stylesheet" href="<?=base_url('assets/css/frontStyle.css')?>">
...
<?php if (session()->getFlashdata('success') || session()->getFlashdata('errors') || session()->getFlashdata('error')): ?>
  <div id="alert-container" class="w3-top" style="position: fixed; top: 0; width: 100%; z-index: 10000; padding-top: 15px;">
    <div style="max-width: 1200px; margin: 0 auto;">
      <?php if (session()->getFlashdata('success')): ?>
        <div class="w3-panel w3-pale-green w3-card w3-round-large" style="position: relative; margin-bottom: 15px; border-left: 4px solid #4CAF50; animation: slideDown 0.4s ease-out;">
      ...
      <?php if (session()->getFlashdata('errors')): ?>
        <div class="w3-panel w3-pale-red w3-card w3-round-large" style="position: relative; margin-bottom: 15px; border-left: 4px solid #f44336; animation: slideDown 0.4s ease-out;">
          <span onclick="this.parentElement.style.display='none'" 
                    ...
                    <div class="footer-widget">
                        ...
                        <div class="footer-text">
                              <?php foreach($legalItems as $item): ?>
                                <a href="<?= esc($item['url']) ?>" class="w3-text-white"><?= esc($item['title_en']) ?></a><br>
                              <?php endforeach; ?>
                        </div>
                    </div>
                </div>
                          ...
                          <div class="footer-text">
                              <?php if (!empty($serviceItems)): ?>
                                <?php foreach ($serviceItems as $item): ?>
                                    <p>
                                        <a href="<?= esc($item['url']) ?>" class="text-white">
                                            <?= esc($item['title_en']) ?><br>
                                        </a>
                                    </p>
                                <?php endforeach; ?>
                                <?php else: ?>
                                    <p><?= lang('app.no_items') ?></p>
                                <?php endif; ?>
                          </div>
							          </div>
                      </div>
                ...
                <ul>
                  <?php if (!empty($linkItems)): ?>
                      <?php foreach ($linkItems as $item): ?>
                          <li>
                              <a href="<?= esc($item['url']) ?>" class="w3-text-white"><?= esc($item['title_en']) ?></a>
                          </li>
                      <?php endforeach; ?>
                  <?php else: ?>
                      <li>Item Not Exist!</li>
                  <?php endif; ?>
                </ul>
