<div class="card mb-4">
    <div class="card-header">
        <h4><?=lang('app.all_businesses')?></h4>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="container-fluid mt-5">
                <form id="myForm" data-url="<?= base_url('businesses/all') ?>">
                    <?= csrf_field() ?>
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination justify-content-center">
                        ...
                        <li class="page-item <?= !$pagination['has_previous'] ? 'disabled' : '' ?>">
                            ...
                            onclick="<?= $pagination['has_previous'] ? "loadMe('".base_url('businesses/all?search='.esc($search_term).'&page='.($pagination['current_page'] - 1))."')" : '' ?>"
                            aria-label="Previous">
                        ...
                        <?php for ($i = 1; $i <= $pagination['total_pages']; $i++): ?>
                            <li class="page-item <?= $i == $pagination['current_page'] ? 'active' : '' ?>">
                                <a class="page-link" 
                                href="javascript:void(0);" 
                                onclick="loadMe('<?=base_url('businesses/all?search='.esc($search_term).'&page='.$i)?>')">
                                    <?= $i ?>
                                </a>
                            </li>
                        <?php endfor; ?>
                        ...
                        <li class="page-item <?= !$pagination['has_next'] ? 'disabled' : '' ?>">
                            <a class="page-link" 
                            href="javascript:void(0);" 
                            onclick="<?= $pagination['has_next'] ? "loadMe('" . base_url('businesses/all?search='.esc($search_term).'&page=' . ($pagination['current_page'] + 1)) . "')" : '' ?>"
                            aria-label="Next">
